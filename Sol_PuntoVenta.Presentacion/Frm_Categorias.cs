...
namespace Sol_PuntoVenta.Presentacion
{
    public partial class Frm_Categorias : Form
    {
        ...
        private void Formato()
        {
            ...
            Dgv_Listado.Columns[1].HeaderText = "CÓDIGO CA";
        ...
        private void Mostrar(string Ctexto)
        {
            try
            {
                Dgv_Listado.DataSource = N_Categorias.Mostrar_ca(Ctexto);
                this.Formato();
        ...
        {
            if (String.IsNullOrEmpty(Convert.ToString(Dgv_Listado.CurrentRow.Cells["codigo_ca"].Value)))
            {
        ...
        private void Btn_guardar_Click(object sender, EventArgs e)
        {
            ...
            { 
                ...
                {
                    E_Categorias Eca = new E_Categorias();
                    string Cduplica, Rpta = "";
                    Eca.Codigo_ca = Codigo_ca;
                    Eca.Descripcion_ca = Txt_descripcion_ca.Text.Trim();
                    Eca.Estado = true;
                    // Verificamos si la información que se intenta guardar ya existe en la tabla
                    Cduplica = N_Categorias.Verifica_duplicado_ca(this.EstadoGuarda, Eca.Codigo_ca, Eca.Descripcion_ca);
                    if (Cduplica == "")
                    {
                        Rpta = N_Categorias.Guardar_ca(this.EstadoGuarda, Eca);
                        if (Rpta.Equals("OK"))
                        {
                ...
                if (Opcion== DialogResult.OK)
                {
                    ...
                    foreach (DataGridViewRow row in Dgv_Listado.Rows)
                    {
                        if (Convert.ToBoolean(row.Cells[0].Value))
                        {
                            ...
                            Rpta = N_Categorias.Eliminar_ca(Codigo);
                            if (Rpta.Equals("OK"))
                            {
        ...
        {
            Reportes.Frm_Rpt_Categorias ORpt_ca = new Reportes.Frm_Rpt_Categorias();
            ORpt_ca.Txt_p1.Text = Txt_buscar.Text;
            ORpt_ca.ShowDialog();
        }
    }
}
