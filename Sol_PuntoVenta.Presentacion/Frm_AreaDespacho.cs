...
{
    public partial class Frm_AreaDespacho : Form
    {
        public Frm_AreaDespacho()
        {
           InitializeComponent();
        }
        ...
        private void Formato()
        {
            ...
            Dgv_Listado.Columns[1].HeaderText = "CÓDIGO AD";
            Dgv_Listado.Columns[2].Width = 300;
            Dgv_Listado.Columns[2].HeaderText = "PUNTO VENTA";
            Dgv_Listado.Columns[3].Width = 200;
            Dgv_Listado.Columns[3].HeaderText = "AREA DESPACHO";
            Dgv_Listado.Columns[4].Width = 320;
            Dgv_Listado.Columns[4].HeaderText = "IMPRESORA";
        ...
        private void Mostrar(string Ctexto)
        {
            try
            {
                Dgv_Listado.DataSource = N_AreaDespacho.Mostrar_ad(Ctexto);
                this.Formato();
        ...
        private void Selecciona_dgv()
        {
            ...
            {
                Codigo_ad = Convert.ToInt32(Dgv_Listado.CurrentRow.Cells["codigo_ad"].Value);
        ...
        private void Listar_pv(string Ctexto)
        {
            try
            {
                Cmb_PuntodeVenta.DataSource = N_AreaDespacho.Listar_pv(Ctexto);
        ...
        private void Estado_Limpiar(bool Lestado)
        {
            ...
            Txt_buscar.Clear();            
            Txt_descripcion_ad.Clear();
            Txt_impresora.Clear();
            Txt_descripcion_ad.ReadOnly = !Lestado;
        ...
        private void Frm_AreaDespacho_Load(object sender, EventArgs e)
        {
            this.Listar_pv("%");
            this.Mostrar("%");
        }
        ...
        private void Btn_guardar_Click(object sender, EventArgs e)
        {
            try
            { 
                if (Txt_descripcion_ad.Text== string.Empty ||
                ...
                {
                    E_AreaDespacho Ead = new E_AreaDespacho();
                    string Cduplica, Rpta = "";
                    Ead.Codigo_ad = Codigo_ad;
                    Ead.Codigo_pv= Convert.ToInt32(Cmb_PuntodeVenta.SelectedValue);
                    Ead.Descripcion_ad = Txt_descripcion_ad.Text.Trim();
                    Ead.Impresora = Txt_impresora.Text.Trim();
                    Ead.Estado = true;
                    // Verificamos si la información que se intenta guardar ya existe en la tabla
                    Cduplica = N_AreaDespacho.Verifica_duplicado_ad(this.EstadoGuarda, Ead.Codigo_ad, Ead.Descripcion_ad);
                    if (Cduplica == "")
                    {
                        Rpta = N_AreaDespacho.Guardar_ad(this.EstadoGuarda, Ead);
                        if (Rpta.Equals("OK"))
                        {
                ...
                if (Opcion== DialogResult.OK)
                {
                    ...
                    foreach (DataGridViewRow row in Dgv_Listado.Rows)
                    {
                        if (Convert.ToBoolean(row.Cells[0].Value))
                        {
                            ...
                            Rpta = N_AreaDespacho.Eliminar_ad(Codigo);
                            if (Rpta.Equals("OK"))
                            {
        ...
        {
            Reportes.Frm_Rpt_AreaDespacho ORpt_ad = new Reportes.Frm_Rpt_AreaDespacho();
            ORpt_ad.Txt_p1.Text = Txt_buscar.Text;
            ORpt_ad.ShowDialog();
        }       
